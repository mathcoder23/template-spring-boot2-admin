...
public class Session {
    ...
    public ChannelFuture sendText(ByteBuffer byteBuffer) {
        ...
        return channel.writeAndFlush(new TextWebSocketFrame(buffer.writeBytes(byteBuffer)));
    }
    ...
    public ChannelFuture sendBinary(byte[] bytes, int offset, int length) {
        ByteBuf buffer = channel.alloc().buffer(length);
        return channel.writeAndFlush(new BinaryWebSocketFrame(buffer.writeBytes(bytes,offset,length)));
    }
